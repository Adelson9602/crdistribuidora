...
export default {
  ...
  methods: {
    ...
    getData(){
      ...
      setTimeout(async() => {
        try {
          const res_credits = await this.getAllCredits(this.data_user).then( res => {
            return res.data;
          });
          console.log({
            ...
            if(res_credits.result){
              ...
              res_credits.data.forEach( credit => {
                this.data.push({
                  ...
                  Ev_Id: credit.Ev_Id,
                  Ev_Total_venta: `$ ${formatter.format(credit.Ev_Total_venta)}`,
                  ...
                  conf_pago: credit.pdt_conf_pago ? 'Tiene pagos por confirmar' : 'No tiene pagos por confirmar', //0 sin pagos por confirmar y 1 es con pagos por confirmar
