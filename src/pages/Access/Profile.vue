  ...
  methods: {
    ...
    reload() {
     ...
     async onSubmit() {
      ...
      }else if(this.dataUsuario.Password == this.dataUsuario.repeat_pass && this.dataUsuario.Codigo != this.dataUsuario.repeat_codigo){
        this.$q.notify({
          color: "negative",
          icon: "warning",
    ...
    async apiRegister() {
      ...
      const resPostLogin = await this.login(setDataUser).then( res => {
        return res.data;
      }).catch(e => {
        throw new Error(e.message);
      })
