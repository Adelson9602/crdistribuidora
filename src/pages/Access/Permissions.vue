            ...
            <q-card-section class="row items-center">
              <div class="col-xs-12 col-md-6 q-pa-xs q-gutter-y-md">
                <q-input
                  filled
                  maxlength="100"
                  v-model="permiso.label"
                  hint="Nombre de modulo"
              ...
              <div class="col-xs-12 col-md-6 q-pa-xs q-gutter-y-md">
                <q-input
                  ...
                  hint="Descripción de modulo"
                  :rules="[
                    val => !!val || 'Descripción de módulo es obligatorio'
                  ]"
                />
              </div>
              <div class="col-xs-12 col-md-6 q-pa-xs q-gutter-y-md">
            ...
            <q-scroll-area style="height: 70vh">
              <q-list bordered class="rounded-borders">
                <q-item-label header class="text-h6"
                  >Items asociados</q-item-label
                >
                <q-expansion-item
                  v-for="item in items_modulo"
                  :key="item.Id_item_modulo"
                >
                  <template v-slot:header>
                    ...
                    <q-item-section side>
                      <div class="row items-center q-gutter-sm">
                        <q-icon
                          name="info"
                          :color="item.asociar ? 'green' : 'warning'"
                        />
              ...
              <q-list bordered class="rounded-borders">
                <q-item-label header class="text-h6"
                  >Items sin asociar</q-item-label
                >
                <q-expansion-item
                  v-for="item in items_no_modulo"
                  :key="item.Id_item_modulo"
                >
                  <template v-slot:header>
                    <q-item-section>
...
export default {
  ...
  data() {
    return {
      ...
      optionsType: ["expantion-item", "item", "dialog"],
      options_state: [
        {
          value: 1,
          label: "ACTIVO"
  ...
  methods: {
    ...
    async addPermiso() {
      ...
      try {
        ...
        const res_insert = await this.PostCreateModules(this.permiso).then(
          res => {
            return res.data;
          }
        );
    ...
    async addItems() {
      ...
      try {
        ...
        this.items_no_modulo.forEach(element => {
          ...
          element.Id_modulo = element.asociar
            ? element.new_Id_modulo
            : element.Id_modulo;
          element.base = process.env.__BASE__;
          const ins_itm = this.PostCreateItemModules(element).then(res => {
            return res.data;
    ...
    detailsModulo(row) {
      ...
      let item_modulo = all_items_modulos.filter(
        item => item.Id_modulo == row.Id_modulo && item.Estado == 1
      );
      let item_no_modulo = all_items_modulos.filter(
        item => item.Id_modulo != row.Id_modulo || item.Estado == 0
      );
