<template>
  <q-page padding>
    <q-card class="q-pa-md">
      <q-form @submit="getStockOnlyMovil">
...
export default {
  ...
  methods: {
    ...mapActions("warehouse", ["getStock", "getStockMovil"]),
    ...mapActions("master", ["getMovil"]),
    async getData() {
      this.$q.loading.show({
    ...
    async getStockOnlyMovil() {
      this.$q.loading.show({
        message: "Obteniendo inventario, por favor espere..."
      });
      try {
        const res_stock = await this.getStockMovil(this.movil_selecte).then(
          res => {
            return res.data;
          }
        );
        ...
        update(
          () => {
            if (val === "") {
              ...
              this.options_movil = all_movils.filter(
                v => v.lable.toLowerCase().indexOf(needle) > -1
              );
            }
          },
