<template>
  <div>
    <div class="row q-gutter-y-md">
      ...
      <q-form
        @submit="searchCotizacion"
        class="col-xs-12 col-sm-4 col-md-4 q-px-sm"
      >
        <q-input
          bottom-slots
    ...
    >
      ...
      <div class="row">
        ...
        <div class="col-xs-12 col-sm-6 col-md-3 q-px-sm">
          <q-input
            hint="Fecha venta"
            v-model="enc_venta.Ev_Fecha_venta"
            :rules="[val => !!val || 'Ingrese fecha de venta']"
          >
      ...
      <template v-slot:body="props">
        <q-tr :props="props">
          ...
          <q-td v-for="col in props.cols" :key="col.name" :props="props">
            {{
              col.name != "Dv_precio_venta" && col.name != "subtotal_product"
                ? col.value
...
export default {
  ...
  methods: {
    ...
    ...mapActions("shopping", ["getProviders", "getTpDoc"]),
    ...
    async getData() {
      ...
      try {
        ...
        const res_mov = await this.getMovilUser(
        ...
        if (res_por_prod.ok) {
          ...
          res_por_prod.data.forEach(element => {
            if (
              element.Estado == 1 &&
              this.data_user.Per_Num_documento == element.Integrante
            ) {
              percent_genres.push({
                label: element.Pv_Descripcion,
                value: element.Pv_Prcentaje,
    ...
    async onSubmit() {
      ...
      try {
        ...
        if (this.tipo_accion) {
          ...
          this.enc_venta.Ev_Des_gen_venta =
            (this.enc_venta.Ev_Subtotal - this.enc_venta.Ev_Des_total_art) *
            (this.enc_venta.Ev_Descuentog / 100);
          this.enc_venta.Ev_Total_venta =
            this.enc_venta.Ev_Subtotal -
          ...
          this.enc_venta.Ev_Total_venta =
            this.enc_venta.Ev_Impuesto > 0
              ? this.enc_venta.Ev_Total_venta * this.enc_venta.Ev_Impuesto
              : this.enc_venta.Ev_Total_venta;
          const res_enc = await this.insertEncVenta(this.enc_venta).then(
            res => {
              return res.data;
            }
          );
          ...
          if (res_enc.ok) {
            ...
            this.data_sales.forEach(product => {
              ...
              promesas.push(
                this.insertDetVenta(product).then(res => {
              ...
              if (product.cantidad_garantia) {
                ...
                promesas.push(
                  this.insertUpdateEncGarantia(this.ecn_garantia)
                    .then(res => {
          ...
          if (this.cliente_selected.value == 0) {
            ...
            enc_cotizacion.Ec_Total_venta =
              this.enc_venta.Ev_Impuesto > 0
                ? enc_cotizacion.Ec_Total_venta * this.enc_venta.Ev_Impuesto
                : enc_cotizacion.Ec_Total_venta;
            const res_enc = await this.insertEncCotizacion(enc_cotizacion).then(
              res => {
                return res.data;
              }
            );
            ...
            if (res_enc.ok) {
              ...
              this.data_sales.forEach(product => {
                ...
                promesas.push(
                  this.insertDetCotizacion(product).then(res => {
    ...
    addProduct() {
      ...
      if (
        this.cantidad > this.cant_disponible ||
        Number(this.cantidad) + Number(this.cantidad_garantia) >
        ...
        if (exits_product) {
          ...
          this.total_venta = Math.round(
            this.enc_venta.Ev_Impuesto > 0
              ? this.total_venta * this.enc_venta.Ev_Impuesto
              : this.total_venta
          );
    ...
    async searchCotizacion() {
      if (this.id_cotizacion) {
        ...
        try {
          const res_enc = await this.getEncCotizacion(this.id_cotizacion).then(
            res => {
              return res.data;
            }
          );
          ...
          if (res_enc.ok) {
            if (res_enc.result) {
              ...
              this.cliente_selected = all_clients.find(
                cliente => cliente.value == encabezado.CP_Nit
              );
              ...
              const res_det = await this.getDetCotizacion(
                this.id_cotizacion
              ).then(res => {
                return res.data;
              });
                ...
                if (res_det.result) {
                  res_det.data.forEach(product => {
                    ...
                    product_add.subtotal_product = Math.round(
                      (product_add.Dv_Precio_compra +
                        product_add.Dv_Precio_compra *
                          (product_add.Dv_valor_descuento / 100)) *
                        product_add.Dv_Cant
                    ); //calcula el subtotal por cada articulo
                    this.Ev_Des_total_art = Math.round(
                      this.Ev_Des_total_art +
                        (product_add.Dv_precio_venta * product_add.Dv_Cant -
                          product_add.subtotal_product)
                    ); //Calculamos el descuento de cada articulo
                    product_add.diferencia_descuento = this.Ev_Des_total_art;
