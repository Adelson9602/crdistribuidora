<template>
  <div>
    ...
    <q-form @submit="addProduct" class="q-gutter-md" ref="form_sales">
      <!-- Productos -->
      <div class="row">
    ...
    <q-page-sticky
      ...
      :offset="[18, 18]"
      v-if="data_products_sales.length > 0"
    >
  ...
  watch: {
    producto_selecte(value) {
      if (value) {
        ...
        this.precio_vendido = new Intl.NumberFormat().format(
          value.precio_venta
        );
      }
    }
  ...
  methods: {
    ...
    async onSumit() {
      ...
      try {
        ...
        this.enc_nota_credito.Ev_Des_gen_venta =
          (this.enc_nota_credito.Ev_Subtotal -
            this.enc_nota_credito.Ev_Des_total_art) *
          (this.enc_nota_credito.Ev_Descuentog / 100);
        this.enc_nota_credito.Ev_Total_venta =
          this.enc_nota_credito.Ev_Subtotal -
        ...
        const res_enc_credito = await this.insertEncNotaCredito(
          this.enc_nota_credito
        ).then(res => {
          return res.data;
        });
        // console.log({
        ...
        if (res_enc_credito.ok) {
          ...
          this.data_products_sales.forEach(product => {
            ...
            promesas.push(
              this.insertDetNotaCredito(product)
                .then(res => {
                ...
                .catch(e => {
                  throw new Error(
                    "Error al guardar el detalle de la nota crédito"
                  );
                })
            );
            promesas.push(
      ...
      if (this.cantidad > this.producto_selecte.cantidad) {
        ...
        product_add.subtotal_product = Math.round(
          (product_add.Dv_Precio_compra +
            product_add.porcentaje_venta *
              (product_add.Dv_Precio_compra / 100)) *
            this.cantidad
