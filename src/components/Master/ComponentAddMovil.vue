      ...
      <div class="row q-gutter-y-md">
        <div class="col-xs-12 col-sm-6 q-px-sm">
          <q-input
            ...
            @input="
              val => {
                movil.Mov_Descripcion = val.toUpperCase();
              }
            "
          />
        </div>
          ...
          <q-tr :props="props">
            <q-td auto-width>
              <q-btn
                ...
                size="sm"
                :color="
                  props.row.Estado == 1 && props.row.agregado
                    ? 'green'
                    : 'negative'
                "
                round
...
export default {
  ...
  methods: {
    ...
    async getData() {
      ...
      try {
        ...
        if (this.edit_data) {
          ...
          const res_integrantes = await this.getMembersMovil(
            this.edit_data.Mov_Id
          ).then(res => {
            return res.data;
          });
          // console.log({
          ...
          if (res_integrantes.ok) {
            if (res_integrantes.result) {
              ...
              res_integrantes.data.forEach(persona => {
                let persona_added = all_persons.find(
                  user => user.value == persona.Per_Num_documento
                );
      ...
      try {
        ...
        if (res_mov.ok) {
          ...
          this.data_integrantes.forEach(pesona => {
            ...
            promesas.push(
              this.insertIntegranteMovil(pesona).then(res => {
                return res.data;
              })
            );
          });
    ...
    addPerson() {
      ...
      let person_added = this.data_integrantes.find(
        person => person.Per_Num_documento == this.person_selected.value
      );
      if (person_added) {
        this.$q.notify({
    ...
    deltePerson(row) {
      ...
      this.$q
        .dialog({
          ...
          parent: this,
          title: `${
            row.agregado && row.Estado == 1
              ? "Inactivar integrante"
              : row.agregado && row.Estado == 0
              ? "Activar integrante"
              : "Eliminar persona"
          }`,
          msg: `Este usuario ya está ${
            row.agregado ? "asociado a la móvil" : "agregado a la tabla"
          }, vas a ${
            row.agregado && row.Estado == 1
              ? "inactivar"
              : row.agregado && row.Estado == 0
              ? "activar"
              : "eliminar"
          } un integrante agregado a la ${
            row.agregado ? "móvil" : "tabla"
        ...
        .onOk(async () => {
          if (this.edit_data) {
            ...
            try {
              ...
              const res_update = await this.insertIntegranteMovil(row).then(
                res => {
                  return res.data;
                }
              );
        ...
        update(
          () => {
            if (val === "") {
              ...
              this.options_person = all_persons.filter(
