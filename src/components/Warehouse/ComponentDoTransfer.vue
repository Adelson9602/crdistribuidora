    ...
    >
      <div class="row">
        ...
        <div
          ...
          v-if="enc_traslado.Etm_Estado == 1"
        >
          <q-select
...
export default {
      ...
      if (
        ...
        this.$q
          .dialog({
            ...
            msg:
              "Atención! Estas cambiando el valor del select, esto borrará todos los datos agregados hasta ahora, ¿Está seguro que desea continuar?"
          })
          .onOk(() => {
            this.onReset();
  ...
  methods: {
    ...
    ...mapActions("master", ["getMovil", "getMembersMovil"]),
    async getData() {
      this.$q.loading.show({
    ...
    async onSubmit() {
      ...
      try {
        ...
        const res_enc = await this.insertEncTransfer(this.enc_traslado).then(
          res => {
            return res.data;
          }
        );
    ...
    addProduct() {
      let product_add = {
        ...
        integ_movil_destino:
          this.enc_traslado.Etm_Estado == 1
            ? this.integ_movil_destino.label
            : 0,
      ...
      let exist_product = this.data_transfer.find(
        product => product.prod_codigo == product_add.prod_codigo
      );
      if (exist_product) {
        this.$q.notify({
    ...
    async getDataMovilOrigen(value) {
      ...
      try {
        const member_movil = await this.getMembersMovil(value.value).then(
          res => {
            return res.data;
          }
        );
    ...
    async getDataMovilDestino(value) {
      ...
      return new Promise((resolve, reject) => {
        setTimeout(() => {
          if (this.cantidad_disponible < val) {
            resolve(
              false || "La cantidad a trasladar es mayor a la disponible"
            );
          } else if (!val) {
        ...
        update(
          () => {
            if (val === "") {
              ...
              this.opt_products = opt_products_transfer.filter(
                v =>
