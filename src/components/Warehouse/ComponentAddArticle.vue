<template>
  <div>
    <q-form @submit="addProduct" autocomplete="off" ref="form_article">
      <div class="row q-gutter-y-md">
        <div
          v-if="this.edit_data ? true : false"
          class="col-xs-12 col-sm-6 col-md-6 q-px-sm"
        >
          <q-input
            @input="
              val => {
                new_article.Art_Codigo_inv = val.toUpperCase();
              }
            "
        ...
        <div class="col-xs-12 col-sm-6 col-md-6 q-px-sm">
          <q-input
            @input="
              val => {
                new_article.Art_Nombre = val.toUpperCase();
              }
            "
        ...
        <div class="col-xs-12 col-sm-6 col-md-6 q-px-sm">
          <q-input
            @input="
              val => {
                new_article.Art_Descripcion = val.toUpperCase();
              }
            "
        ...
        <div class="col-xs-12 col-sm-6 col-md-4 q-px-sm">
          <q-input
            @input="
              val => {
                new_article.Art_ubicacion = val.toUpperCase();
              }
            "
        ...
        <div class="col-xs-12 col-sm-6 col-md-4 q-px-sm">
          <q-file v-model="img_articulo" hint="Imagen artículo" counter>
...
export default {
  ...
  methods: {
    ...mapActions("warehouse", ["getCategoriasAlmacen", "addArticle"]),
    ...mapActions("master", ["getAllUm"]),
    ...mapActions("access", ["saveFile"]),
    async getData() {
      this.$q.loading.show({
      ...
      try {
        ...
        if (this.edit_data) {
          ...
          let categoria = options_categorias.find(
            categoria =>
              categoria.label.toLowerCase() ==
              this.edit_data.Cat_Nombre.toLowerCase()
          );
          // Buscamos la unidad de medida asiganada
          let um = options_um.find(
            um =>
              um.prefijo.toLowerCase() == this.edit_data.Prefijo.toLowerCase()
          );
          this.new_article = {
            base: null,
            ...
            Art_Imagen: this.edit_data.Art_Imagen
              ? this.edit_data.Art_Imagen
              : this.new_article.Art_Imagen,
        ...
        this.data_articles.forEach(articulo => {
          ...
          if (articulo.tem_img) {
            ...
            formData.append(
              "files",
              articulo.tem_img,
              articulo.tem_img.newname
            );
            promesas.push(
              this.saveFile(formData).then(async res => {
          ...
          promesas.push(
            this.addArticle(articulo).then(res => {
        ...
        update(
          () => {
            if (val === "") {
              ...
              this.options_categorias = options_categorias.filter(
        ...
        update(
          () => {
            if (val === "") {
              ...
              const needle = val.toLowerCase();
              this.options_um = options_um.filter(
                v => v.label.toLowerCase().indexOf(needle) > -1
              );
            }
          },
